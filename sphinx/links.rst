
.. following are all the general links referenced in the text in alphabetical order

.. _BirdBrain Snap! tutorial: http://finchrobot.com/software/snap
.. _BirdBrain Technologies: http://www.birdbraintechnologies.com
.. _CC-BY-SA: http://creativecommons.org/licenses/by-sa/4.0/
.. _Carnegie Mellon University: http://www.cmu.edu
.. _Chrome Finch Connection App: https://chrome.google.com/webstore/detail/finch-connection-app/ojocioejjpmeccjfjlhkepchminjemod
.. _Dr. Garth Zeglin: http://www.cs.cmu.edu/~garthz
.. _Finch Chromebook Support: http://finchrobot.com/chromebook-support
.. _Finch Drama Project: http://personalrobotics.github.io/finchdrama
.. _Finch Robot: http://finchrobot.com
.. _Finch Snap Curricula: http://www.finchrobot.com/snap-and-scratch-20-finch-resources
.. _Personal Robotics Lab: http://personalrobotics.ri.cmu.edu
.. _Robotics Institute: http://www.ri.cmu.edu
.. _School of Computer Science: http://www.cs.cmu.edu
.. _Scratch: https://scratch.mit.edu/
.. _Snap Manual: http://snap.berkeley.edu/SnapManual.pdf
.. _Snap!: http://snap.berkeley.edu
.. _run Snap!: http://snap.berkeley.edu/run
.. _blackboard system: https://en.wikipedia.org/wiki/Blackboard_system
.. _github repository: https://github.com/personalrobotics/finchdrama
.. _key-value store: https://en.wikipedia.org/wiki/Key-value_database
.. _Snap! source code: https://github.com/jmoenig/Snap--Build-Your-Own-Blocks

.. following are the links to the Snap sketches on the cloud server
.. be sure to add an entry in code.rst for any new sketches
.. _Finch-Chrome: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=birdbraintech&ProjectName=Finch-Chrome

.. _Finch-Animation-Deck: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Animation-Deck
.. _Finch-Box-Navigation: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Box-Navigation
.. _Finch-Coordination-Intro: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Coordination-Intro
.. _Finch-Drama-Library: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Drama-Library
.. _Finch-Precise-Coordination: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Precise-Coordination
.. _Finch-Sensorless-Parking: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Sensorless-Parking
.. _Finch-Sequencing: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-Sequencing
.. _Finch-State-Machine: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Finch-State-Machine
.. _Sprite-Communication-Intro: http://snap.berkeley.edu/snapsource/snap.html#cloud:Username=prldrama&ProjectName=Sprite-Communication-Intro
